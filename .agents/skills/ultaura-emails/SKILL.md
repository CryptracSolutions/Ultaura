...
description: |
  ...
  - Converting inline HTML emails to React Email templates
  - Creating or editing Supabase auth email templates in supabase/templates/
...
4. **Container width is 560px.** No exceptions.
5. **Border color is `brandColors.border` (`#E7E5E4`).** Not `#eaeaea`.
6. **Logo is a PNG** (`public/logos/logo-email.png`), displayed at 48x48px. SVGs don't render in most email clients.
...
Email-safe font stack (set once in `EmailLayout` via the `<Body>` style, not per template):
...
- **Email asset:** `public/logos/logo-email.png` (must be created — convert SVG to PNG at 96x96px for retina, display at 48x48)
- **Display:** 48x48px, centered, in the layout header
- **Alt text:** `"Ultaura"`
...
**File:** `src/lib/emails/components/email-layout.tsx` (does not exist yet — agents must create it when first needed)
...
interface EmailLayoutProps {
  ...
  signOff?: string;         // Default: '-- Ultaura'
...
2. **`<Container>`** — `w-[560px]`, white bg, `brandColors.border` border, `rounded-lg`, `p-[24px]`, `my-[32px]`
3. **Logo** — `<Img>` 48x48, centered, `alt="Ultaura"`
4. **`{children}`** — template-specific content
5. **Sign-off** — `text-[14px] text-stone-700 mt-[18px]`, default `"-- Ultaura"`
...
3. Remove sign-off if it matches default (`"-- Ultaura"`)
      ...
      <Tailwind>
        <Body className="bg-stone-50 my-auto mx-auto font-sans">
          <Container className="border border-solid border-[#e7e5e4] rounded-lg my-[32px] mx-auto p-[24px] w-[560px] bg-white">
            ...
            {/* ... content ... */}
            <Text className="text-[14px] text-stone-700 mt-[18px] mb-0">-- Ultaura</Text>
          </Container>
        </Body>
      </Tailwind>
    </Html>,
...
3. Use Go template variables for dynamic content
4. Add the `[auth.email.template.{name}]` block to `supabase/config.toml` (see section 3 for exact syntax)
5. Test locally: auth actions send emails to Inbucket at `http://localhost:54324`
...
2. [ ] Logo displays at 48x48px with `alt="Ultaura"`
...
8. [ ] Sign-off present (default `"-- Ultaura"` or justified custom)
...
12. [ ] TypeScript compiles (`pnpm tsc --noEmit`)
13. [ ] Visual check in Inbucket if local dev available (`http://localhost:54324`)
...
| "Best, The X Team" sign-off | Default: `"-- Ultaura"` |
