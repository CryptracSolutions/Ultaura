...
### Tweaking document retrieval

Add the following environment variables to your environment variables:

- `CHATBOT_MAX_DOCUMENTS` - The maximum number of documents to retrieve from 
  the database for retrieving the context. The default value is `3`. The 
  higher this is, the more it can take to reply to a question.
- `CHATBOT_SIMILARITY_THRESHOLD` - The similarity threshold for matching 
  documents. The default value is `0.8`.

Tweak the above to get the best results for your website - while 
compromising the speed of the response time.
