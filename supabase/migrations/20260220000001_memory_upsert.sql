-- Deactivate duplicate active memories per line/key (case-insensitive)
with ranked as (
  select
    id,
    row_number() over (
      partition by line_id, lower(key)
...
-- Enforce single active memory per line/key (case-insensitive)
create unique index if not exists idx_ultaura_memories_line_key_active
  on ultaura_memories (line_id, lower(key))
  where active = true;

-- Atomic upsert for memory updates (deactivate + insert new version)
