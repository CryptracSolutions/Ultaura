...
export const PhoneFrame = ({ children, delay = 0, animateIn = true, scale = 1, tilt3D = true, tiltIntensity = 1, glowPulse = true, }) => {
    ...
    const tiltX = tilt3D
        ? interpolate(Math.sin(adjustedFrame * 0.03), [-1, 1], [-3 * tiltIntensity, 3 * tiltIntensity])
        : 0;
    const tiltY = tilt3D
        ? interpolate(Math.cos(adjustedFrame * 0.025), [-1, 1], [-2 * tiltIntensity, 2 * tiltIntensity])
        : 0;
    ...
    const reflectionX = interpolate(adjustedFrame % 180, [0, 90, 180], [-100, 200, -100], { easing: easings.smooth });
