...
interface GradientBackgroundProps {
  variant?: "default" | "radial" | "mesh" | "aurora";
  animated?: boolean;
...
const Particle: React.FC<{
  ...
  const opacity = interpolate(
    cycleFrame,
    [0, duration * 0.15, duration * 0.7, duration],
    [0, 0.4 + (index % 3) * 0.1, 0.4 + (index % 3) * 0.1, 0],
    { extrapolateLeft: "clamp", extrapolateRight: "clamp" }
  );

  // Glow pulse
  const glowIntensity = interpolate(
    Math.sin(adjustedFrame * 0.08 + index),
    [-1, 1],
    [0.5, 1]
  );
...
export const GradientBackground: React.FC<GradientBackgroundProps> = ({
  ...
  return (
    ...
    >
      ...
      {showNoise && <NoiseOverlay opacity={0.025} />}
