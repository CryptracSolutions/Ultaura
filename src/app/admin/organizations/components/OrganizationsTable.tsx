  ...
  return (
    <TableContainer>
      {organizations.length === 0 ? (
        <TableEmptyState message="No organizations found." />
      ) : (
        <Table data-cy="admin-organizations-table">
          <TableHeader>
            <TableRow>
          ...
          <TableBody>
            {organizations.map((org) => {
              ...
              const price = plan?.plans.find(
                (p) => p.stripePriceId === priceId,
              );
              const subscriptionLabel =
                plan && price ? `${plan.name} - ${price.name}` : '-';
              ...
              if (subscription) {
                ...
                const formattedDate = formatDate(subscription.periodEndsAt);
                periodLabel = canceled ? (
                  <span className="text-orange-500">
                    Stops on {formattedDate}
                  </span>
                ) : (
                    ...
                    <div className="flex justify-end">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem
                            onClick={() =>
                              navigator.clipboard.writeText(org.uuid)
                            }
                          >
                            Copy UUID
                          </DropdownMenuItem>
