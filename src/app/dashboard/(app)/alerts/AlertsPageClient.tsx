...
const ALERTS_INFO_TIP = (
  <div className="flex items-start gap-3 rounded-lg border border-primary/30 bg-primary/5 px-4 py-3.5">
    ...
    <p className="text-xs text-primary leading-snug">
      These alerts are generated by AI based on conversation patterns and are not
...
const ALERTS_TABS: Array<{ value: AlertsTabValue; label: string; path: string }> = [
  { value: 'wellness-alerts', label: 'Wellness Alerts', path: '/dashboard/alerts?tab=wellness-alerts' },
  { value: 'alert-settings', label: 'Alert Settings', path: '/dashboard/alerts?tab=alert-settings' },
];
...
export function AlertsPageClient({ alerts, lines, settings, deliveryEmail, disabled }: AlertsPageClientProps) {
  ...
  const activeTab: AlertsTabValue =
    tabParam === 'wellness-alerts' || tabParam === 'alert-settings' ? tabParam : 'wellness-alerts';
