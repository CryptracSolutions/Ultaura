...
describe('confirmNotificationRecipient', () => {
  ...
  it('does not create or update trusted contacts during confirmation', async () => {
    const existing = {
      id: 'recipient-1',
      account_id: 'acct-1',
      name: 'Mary Trusted',
    ...
    const adminClient = {
      from: vi.fn((table: string) => {
        if (table === 'ultaura_notification_recipients') {
          return {
            select: vi.fn(() => recipientSelectForToken),
    ...
    expect(result.success).toBe(true);
    expect(adminClient.from).not.toHaveBeenCalledWith('ultaura_trusted_contacts');
    expect(adminClient.from).not.toHaveBeenCalledWith('ultaura_consents');
  });
});
