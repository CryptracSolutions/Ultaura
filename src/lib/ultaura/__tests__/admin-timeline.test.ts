...
describe('aggregateTimeline', () => {
  ...
  it('account-only sources (notification_recipient, data_export) ignore lineIds filter', async () => {
    ...
    // Account-only sources should still apply account_id filtering
    const accountFilterOnRecipients = filterCalls.find(
      (c) =>
        c.table === 'ultaura_notification_recipients' &&
        c.method === 'in' &&
        c.args[0] === 'account_id',
    );
    const accountFilterOnExports = filterCalls.find(
      (c) =>
        c.table === 'ultaura_data_export_requests' &&
        c.method === 'in' &&
        c.args[0] === 'account_id',
    );
    expect(accountFilterOnRecipients).toBeDefined();
    expect(accountFilterOnRecipients!.args[1]).toEqual(['acc-1']);
    expect(accountFilterOnExports).toBeDefined();
    expect(accountFilterOnExports!.args[1]).toEqual(['acc-1']);
  ...
  it('only queries requested direct source tables for call_session + safety_event filter', async () => {
    mockResponses['ultaura_call_sessions'] = {
    ...
    // Verify at the query level: for direct-source fetchers, only the requested tables were queried
    expect(fromCalls).toContain('ultaura_call_sessions');
    expect(fromCalls).toContain('ultaura_safety_events');
    // No helper pre-query tables are expected in this specific case
    const unexpectedTables = fromCalls.filter(
      (t) =>
